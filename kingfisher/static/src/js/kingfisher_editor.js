odoo.define('kingfisher.kingfisher_editor_js', function(require) {
    'use strict';
    ...
    ajax.loadXML('/kingfisher/static/src/xml/kingfisher_pro.xml', qweb);

    options.registry.kingfisher_product_category_slider = options.Class.extend({
        ...
        pro_cat_slider: function(type, value) {
            ...
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("kingfisher.king_dynamic_product_category_slider"));
                ...
                ajax.jsonRpc('/kingfisher/pro_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.kingfisher_brand_custom_slider = options.Class.extend({
        ...
        king_brand_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("kingfisher.king_brands_slider_block"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.kingfisher_blog_custom_snippet = options.Class.extend({
        ...
        king_blog_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("kingfisher.king_blog_slider_block"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                ...
                ajax.jsonRpc('/kingfisher/blog_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.kingfisher_multi_cat_custom_snippet = options.Class.extend({
        ...
        multi_category_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("kingfisher.multi_product_custom_slider_block"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                var $slider_type = self.$modal.find("#slider_type"),
                    $cancel = self.$modal.find("#cancel"),
                ...
                ajax.jsonRpc('/kingfisher/product_multi_get_options', 'call', {}).done(function(res) {
