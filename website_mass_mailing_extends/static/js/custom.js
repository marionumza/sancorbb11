  $(document).ready(function () {
      $("#more_button").click(function() {
          $("#more_child").append("<div class=\"row form-group lines\">\n" +
              ...
              "\t\t\t\t\t\t\t\t\t\t<input  name=\"day\"  class=\"form-control\" placeholder=\"DD\" autofocus=\"autofocus\"/>\n" +
              "\t\t\t\t\t\t\t\t\t</div>\n" +
              "\t\t\t\t\t\t\t\t\t<div class=\"col-lg-1 labels\">\n" +
              "\t\t\t\t\t\t\t\t\t\t<label>/</label>\n" +
              "\t\t\t\t\t\t\t\t\t</div>\n" +
              "\t\t\t\t\t\t\t\t\t<div class=\"col-lg-1 date\">\n" +
              "\t\t\t\t\t\t\t\t\t\t<input name=\"month\" class=\"form-control\" placeholder=\"MM\" autofocus=\"autofocus\"/>\n" +
              "\t\t\t\t\t\t\t\t\t</div>\n" +
              "\t\t\t\t\t\t\t\t\t<div class=\"col-lg-1 labels\">\n" +
              "\t\t\t\t\t\t\t\t\t\t<label >/</label>\n" +
              "\t\t\t\t\t\t\t\t\t</div>\n" +
              "\t\t\t\t\t\t\t\t\t<div class=\"col-lg-1 date\">\n" +
              "\t\t\t\t\t\t\t\t\t\t<input  name=\"year\" class=\"form-control\" placeholder=\"AAAA\" autofocus=\"autofocus\"/>\n" +
              "\t\t\t\t\t\t\t\t\t</div>\n" +
              "\t\t\t\t\t\t\t\t</div>");
...
sAnimation.registry.newsletter_popup = sAnimation.Class.extend({
    ...
    _onClickSubscribe: function () {
        ...
        for (var i = 0;i < $day.length;i++){
            var day = $day[i].value;
            var month = $month[i].value;
            var year = $year[i].value;
            if(day!="" || month!="" || year!=""){
                var date = new Date(year, month, day);
                if (date && day == date.getDate() && month == date.getMonth() && year == date.getFullYear()) {
                    day_arr.push(day);
